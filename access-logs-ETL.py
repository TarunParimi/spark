...
print(f"Inserting Data into retail.tokenized_access_logs_partitioned table \n")
cleansed_products_df.\
  write.\
  mode("overwrite").\
  partitionBy("url").\
  saveAsTable("retail"+'.'+"tokenized_access_logs_partitioned", format="parquet")

cleansed_products_df.filter("url == '/department/fitness'").write.\
  mode("overwrite").\
  partitionBy("url").\
  saveAsTable("retail"+'.'+"tokenized_access_logs_partitioned_fitness", format="parquet")

print(f"Count number of records inserted \n")
spark.sql("Select count(*) as RecordCount from retail.tokenized_access_logs_partitioned").show()

print(f"Retrieve 15 records for validation \n")
spark.sql("Select * from retail.tokenized_access_logs_partitioned limit 15").show()

print(f"Retrieve 15 fitness records for validation \n")
spark.sql("Select * from retail.tokenized_access_logs_partitioned_fitness limit 15").show()
